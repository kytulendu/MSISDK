...
*  msicube.c
*
*  This sample code is designed to demonstrate the use of the MSI95 API.
...
#define WM_POST_CREATE1         WM_USER+0x102
#define WM_POST_MAXIMIZED1      WM_USER+0x104
...
float              CubeVerticesY[24];
float              CubeVerticesZ[24];
float              CubeVerticesOX[24];  /* These are the Original X, Y, Z Values. */
float              CubeVerticesOY[24];
float              CubeVerticesOZ[24];
T_msiVertex        CubeVertices[24];    /* Cube Verticies (SCR Coordinates to Msi) */

static LPBYTE pTextures;                /* Pointer to a Texture Heap to hold the cube textures */
static LPBYTE pNumbers;                 /* Pointer to a Texture Heap to hold the number texture */
static LPBYTE pLUT;                     /* Pointer to a Texture Heap to hold the LUT */
...
int                YRotation = 0;       /* Current state of Rotation in Y-Axis. */
int                ZRotation = 0;       /* Current State of Rotation in Z-Axis. */
...
BOOL InitNumbers( ) {
    ...
    Vertices[1].x = 0.0f;   Vertices[1].y = 32.0f;  Vertices[1].z = 0.0f;
    Vertices[1].u = 0.0f;   Vertices[1].v = 1.0f;   Vertices[1].invW = 1.0f;
    Vertices[2].x = 32.0f;  Vertices[2].y = 0.0f;   Vertices[2].z = 0.0f;
    Vertices[2].u = 1.0f;   Vertices[2].v = 0.0f;   Vertices[2].invW = 1.0f;
    Vertices[3].x = 32.0f;  Vertices[3].y = 32.0f;  Vertices[3].z = 0.0f;
    Vertices[3].u = 1.0f;   Vertices[3].v = 1.0f;   Vertices[3].invW = 1.0f;
    Vertices[4].x = 32.0f;  Vertices[4].y = 0.0f;   Vertices[4].z = 0.0f;
    Vertices[4].u = 1.0f;   Vertices[4].v = 0.0f;   Vertices[4].invW = 1.0f;
    Vertices[5].x = 32.0f;  Vertices[5].y = 32.0f;  Vertices[5].z = 0.0f;
    Vertices[5].u = 1.0f;   Vertices[5].v = 1.0f;   Vertices[5].invW = 1.0f;
    Vertices[6].x = 64.0f;  Vertices[6].y = 0.0f;   Vertices[6].z = 0.0f;
    Vertices[6].u = 2.0f;   Vertices[6].v = 0.0f;   Vertices[6].invW = 1.0f;
    Vertices[7].x = 64.0f;  Vertices[7].y = 32.0f;  Vertices[7].z = 0.0f;
    Vertices[7].u = 2.0f;   Vertices[7].v = 1.0f;   Vertices[7].invW = 1.0f;
    Vertices[8].x = 64.0f;  Vertices[8].y = 0.0f;   Vertices[8].z = 0.0f;
    Vertices[8].u = 0.0f;   Vertices[8].v = 0.0f;   Vertices[8].invW = 1.0f;
    Vertices[9].x = 64.0f;  Vertices[9].y = 32.0f;  Vertices[9].z = 0.0f;
    Vertices[9].u = 0.0f;   Vertices[9].v = 1.0f;   Vertices[9].invW = 1.0f;
    Vertices[10].x = 96.0f; Vertices[10].y = 0.0f;  Vertices[10].z = 0.0f;
    Vertices[10].u = 1.0f;  Vertices[10].v = 0.0f;  Vertices[10].invW = 1.0f;
    Vertices[11].x = 96.0f; Vertices[11].y = 32.0f; Vertices[11].z = 0.0f;
    ...
    for ( Again = 0; Again < Repetitions; Again++ ) {
        ...
        for ( Counti = 0; Counti < 24; Counti++ ) {
            ...
            *  deforming after a few spins. */
            ...
            CubeVerticesX[Counti] = CubeVerticesOX[Counti] * CosTable[AngleZ] + CubeVerticesOY[Counti] * SinTable[AngleZ];
            CubeVerticesY[Counti] = CubeVerticesOY[Counti] * CosTable[AngleZ] - CubeVerticesOX[Counti] * SinTable[AngleZ];
            /* Spin around the y-axis */
            OX = CubeVerticesX[Counti];
            CubeVerticesX[Counti] = CubeVerticesX[Counti] * CosTable[AngleY] - CubeVerticesOZ[Counti] * SinTable[AngleY];
            CubeVerticesZ[Counti] = OX * SinTable[AngleY] + CubeVerticesOZ[Counti] * CosTable[AngleY];
            /* Spin around the x-axis */
            OY = CubeVerticesY[Counti];
            CubeVerticesY[Counti] = CubeVerticesY[Counti] * CosTable[AngleX] + CubeVerticesZ[Counti] * SinTable[AngleX];
            CubeVerticesZ[Counti] = CubeVerticesZ[Counti] * CosTable[AngleX] - OY * SinTable[AngleX];
        }
...
int WINAPI WinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow ) {
    ...
    pInfo = msiInit( 640, 480, 16, FALSE, msiDBG_DumpToFile, WindowProc );
    if ( pInfo == NULL ) {
...
*/
LRESULT CALLBACK WindowProc( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam ) {
    static BOOL     TIMER_ON = FALSE;  /* Timer Status. */
    static BOOL     Paused = FALSE;    /* Used to Pause the Cube Drawing on Minimize. */
    int    Counti;
    static int num_timers = 18;

    switch ( uMsg ) {
    case WM_CREATE:
        /* Wait for the Window to fully initialize. */
        PostMessage( hWnd, WM_POST_CREATE1, 0, 0 );
        break;

    case WM_POST_CREATE1:
        ...
        *  Calculation: 6 * 256*256*2bpp/4096 = 192 */
        ...
        /* Many timers to acheive maximum speed of updating the display */
        for ( Counti = 0; Counti < num_timers; Counti++ ) {
            if ( !SetTimer( hWnd, Counti, 0, NULL ) ) {
                DBG_PRINT( "Failed to Set Timers - Too Many are already active..." );
    ...
    case WM_SIZE:
        switch ( wParam ) {
        case SIZE_MAXIMIZED:
            PostMessage( hWnd, WM_POST_MAXIMIZED1, 0, 0 );
            break;
        case SIZE_MINIMIZED:
            Paused = TRUE;
            break;
    ...
    case WM_POST_MAXIMIZED1:
        /* Restore the Texture Cache */
        PutNumbersInCache( );
        PutTexturesInCache( );
    ...
    case WM_KEYDOWN:
        ...
        /* Run the Demo. */
        case VK_HOME:
            TIMER_ON = FALSE;   /* Turn off the timer. */
            ReStartBench( );
            Repetitions = 125;
    ...
    case WM_TIMER:
        if ( !Paused ) {
            ...
            if ( TIMER_ON ) { DrawCube( AngleX, AngleY, AngleZ ); }
        }
        break;
    ...
    return DefWindowProc( hWnd, uMsg, wParam, lParam ); /* Default processing for any window messages
