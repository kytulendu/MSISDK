...
// msimisc.c																					 //
//																									 //
// This sample code demonstrates how some other, less used, MSI95 API		 //
// calls are made.																			 //
...
void RenderZPlaneTest_noZ()
{
   ...
   // Start a new frame and clear the RGB buffer.
   ...
   // Copy the previously loaded bitmaps to the Heap for use in a BlitRect
	...
	while (yy<256*2){
       for (xx=0;xx<256*2;xx++)
	      *(pTexture+640*yy+xx)=pBitmapA[256*yy+xx];
	   yy++;
	}
...
{
	//
	// Demonstrates how to use:	msiQueryInfo
	//
   ...
   // This API call is used to query how much memory is found on the MGA card.
	// This will enable the game designer to have enhanced features on a card
	// with more than 2MB of memory.  As an example, a game developer might want
	// to have better textures on a 4MB game port than on a 2MB.  Or, as a 2nd
	// example, a game might want to take advantage of a Hardware Z buffer that
	// is possible with a 4MB board.
   if(!msiQueryInfo(QueryInfoStructMinSize, (LPBYTE)&BoardInfo))	
   {
    DBG_PRINT("msiQueryInfo has returned an error.");
 	 if (BoardInfo.ErrorStatus & ESCAPECALLNOTSUPPORTED)
	   DBG_PRINT("ErrorStatus: ESCAPECALLNOTSUPPORTED");
	 else if (BoardInfo.ErrorStatus & DRIVERNOTSUPPORTED)
	   DBG_PRINT("ErrorStatus: DRIVERNOTSUPPORTED");
	 else
	   DBG_PRINT("ErrorStatus: BOARDNOTSUPPORTED");
  	 return(FALSE);
   } 
   
   // Query on how much memory is found on the board
   // Example, a 4MB board will have a value of 4*1024*1024 = 4,194,304 bytes
   Amount_Mem = BoardInfo.MemorySize;
   // At this point, the game developer may decide that he might make the game
	// behave differently depending how much memory is found on the MGA card.
	// Now msiInit could be called with the desired mode.  Again, the msiQueryInfo
	// is only usefull if one wants the game to have slightly different graphics
	// depending on the amount of memory found on the board.
